...
def process_file(file, batch_converter=batch_converter, model=model):
    ...
    try:
        ...
        pkl_path = os.path.join(pkl_directory, file + ".pkl")
        ...
        with contextlib.redirect_stdout(null_file):
            protein.import_parameters(
                "./tools/feature_extract/Protein_parameters_setting.json"
            )

        protein.get_descriptor("BLOSUM62")

        node_feature = torch.from_numpy(
            (protein.encodings.values.reshape(-1, 20))
        ).float()
...
def main():
    # 配置logging
    logging.basicConfig(
        filename="./tools/feature_extract/log.log",
        level=logging.ERROR,
